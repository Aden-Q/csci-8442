...
class PATree:
    '''
    Usage:
    1. Create a PATree object with the given points.
    ...
    def __init__(self, points_list, max_dim, dist_func=None) -> None:
        ...
        def build_tree(points_list, node, dim, max_dim):
            ...
            node.dim = dim
            next_dim_left = np.var(vectors[:median][:,:-1], axis=0).argmax()
            node.left = build_tree(vectors[:median], Node(), next_dim_left, max_dim)
            next_dim_right = np.var(vectors[median + 1:][:,:-1], axis=0).argmax()
            node.right = build_tree(vectors[median + 1:], Node(), next_dim_right, max_dim)
...
if __name__ == '__main__':
    ...
    print(pa_tree.root.data)

    query_point = [3, 4.5]
    print(pa_tree.get_knn(query_point, 2))
    print(pa_tree.get_nn(query_point))
